...
* 標題 `#`～`######`（`#` 後面要有空白，`#hashtag` 不算標題）、段落、分隔線 `---`
* 粗體 `**粗**`、斜體 `*斜*`、行內程式碼、程式碼區塊（` ``` ` 或 `~~~`）
* 連結 `[文字](網址)`、圖片 `![說明](網址)`、引用 `>`
  （網址只接受 `http(s)`、`mailto` 與相對路徑，其他像 `javascript:` 會換成 `#`）
* 清單 `-` / `1.`（不支援巢狀）、行尾兩個空白強制換行
* `\*` 這類反斜線跳脫

不支援表格，原始 HTML 會被跳脫、原樣顯示成文字。

`.md` 是唯一會被當成副檔名的東西（不分大小寫），所以節名裡可以有點，例如 `第1.5節` 會原樣顯示。

簡介 `about.txt` 一律當純文字顯示，不支援 Markdown。
